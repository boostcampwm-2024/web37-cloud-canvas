        ...
        actions: {
            ...
            updateNodeLayout: (id, { layoutType, padding }) =>
                set((state) => {
                    ...
                    const newSize = calcParentSizeByChildren(
                        updatedChildren,
                        layoutType,
                        padding,
                    );
                    const sizeDiff = {
                        cols: newSize.cols - node.size['2d'].cols,
                        rows: newSize.rows - node.size['2d'].rows,
                    };

                    const newConnectors = _.mapValues(
                        node.connectors,
